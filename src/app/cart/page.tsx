  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
        <div className="w-20 h-20 rounded-3xl bg-slate-50 border border-slate-200 flex items-center justify-center mx-auto mb-6 text-slate-500">
          <ShoppingBag className="w-10 h-10" />
        </div>
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Your Project Cart is Empty</h1>
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3">
          ...
          <Link
            href="/build"
            className="py-3 px-6 bg-slate-50 hover:bg-[#222222] border border-slate-200 text-slate-900 font-semibold text-xs rounded-xl transition-colors"
          >
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-white border border-slate-200 space-y-2.5">
            <div className="flex items-center justify-between text-xs">
              <div className="flex items-center gap-2 text-slate-600">
                ...
                <span>Campus Delivery: <strong className="text-slate-900">{selectedCollege.name}</strong></span>
              </div>
              {cart.length === 0 ? (
                <span className="text-slate-500 font-medium">
                  Free delivery threshold: <strong className="text-slate-900">₹499</strong>
                </span>
              ) : remainingForFree > 0 ? (
                <span className="text-slate-500 font-medium">
            ...
            <div className="flex items-center justify-between text-[10px] text-slate-500 font-mono">
              ...
              <span className={remainingForFree === 0 ? "text-[#22c55e] font-bold" : "text-slate-500"}>
          ...
          <div className="space-y-3">
            {cart.map((item) => (
              <div
                key={item.id}
                className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                ...
                <div className="flex items-center justify-between w-full sm:w-auto gap-6 shrink-0">
                  ...
                  <button
                    ...
                    className="p-2 text-slate-500 hover:text-[#ef4444] rounded-lg hover:bg-slate-100 transition-colors"
                  >
          ...
          <div className="p-6 rounded-3xl bg-white border border-slate-200 shadow-2xl space-y-6">
            ...
            {!couponCode ? (
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-3 py-2 text-xs text-slate-900 placeholder-neutral-500 focus:outline-none focus:border-[#ff6a00]"
                  />
                </div>
                <button
                  onClick={() => {
                  ...
                  className="px-3.5 py-2 rounded-xl bg-[#222222] hover:bg-[#2c2c2c] text-slate-900 text-xs font-semibold"
                >
                  Apply
                </button>
              </div>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-[#ff6a00]/10 border border-[#ff6a00]/30 text-xs">
                <span className="font-semibold text-slate-900">Coupon {couponCode} applied</span>
                <button onClick={removeCoupon} className="text-slate-500 hover:text-slate-900 underline">
                  Remove
                </button>
              </div>
            )}
            ...
            <div className="space-y-2.5 text-xs text-slate-500">
              ...
              <div className="flex justify-between">
                ...
                <span className={deliveryFee === 0 ? "text-[#22c55e] font-semibold" : "text-slate-900"}>
